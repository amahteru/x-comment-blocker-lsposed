...
if [ ! -f "$TARGET_APK" ]; then
    echo "ℹ️ Target APK not provided in tools/twitter.apk. Using module app itself as test host..."
...
if [ -f "$LSPATCH_JAR" ]; then
    java -jar "$LSPATCH_JAR" "$TARGET_APK" -m "$MODULE_APK" -l 2 -o patched/ || {
        echo "⚠️ LSPatch direct injection returned code $?. Continuing with installed module test..."
    }
fi
