...
def main():
    print("=" * 60)
    print("X Comment Blocker - Local LSPatch Automation Test")
    print("=" * 60)
    ...
    try:
        ...
        if not lines:
            print("[!] No Android device or emulator currently connected.")
            print("[*] Note: Please start an emulator (e.g. MuMu, LDPlayer, or AVD) or connect a phone with USB debugging.")
    except Exception as e:
        print(f"[!] Failed to query ADB devices: {e}")
    ...
    if not os.path.exists(LSPATCH_JAR_PATH) or os.path.getsize(LSPATCH_JAR_PATH) == 0:
        ...
        try:
            ...
            print(f"[!] Download failed ({e}), please manually place lspatch.jar into {TOOLS_DIR}")
